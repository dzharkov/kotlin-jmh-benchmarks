    ...
    open class AVLTree<K, D>(var comparator : Comparator<K?>) {
        open public class Node<K,D>(var key : K, var data : D) {
            ...
            fun updateHeight() : Unit {
                val curLeft = left
                val curRight = right
                val leftHeight = if (curLeft != null) curLeft.height else 0
                val rightHeight = if (curRight != null) curRight.height else 0
    ...
    class OrderAVLTree<K, D>(comparator : Comparator<K?>) : AVLTree<K, D>(comparator) {
        class SizedNode<K,D>(key : K, data : D) : AVLTree<K, D>.Node<K,D>(key, data) {
            ...
            fun updateSize() : Unit {
                val curLeft = left
                val curRight = right
                val leftSize = if (curLeft != null) curLeft.size else 0
                val rightSize = if (curRight != null) curRight.size else 0
